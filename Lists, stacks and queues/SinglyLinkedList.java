...
public class SinglyLinkedList<T> implements Iterable<T> {
	...
	*/
     ...
     * Rearrange the elements of the list by linking the elements at even index
     * followed by the elements at odd index. Implemented by rearranging pointers
     * of existing elements without allocating any new elements. for k > 2
     * @param k value of how apart the elements in a list should be chained
     */
       public void multizip(int k) {
        if (size < 2 * k) {  // Too few elements.  No change.
            return;
        }
       ...
       /**
        * function to iteratively reverse an instance of SinglyLinkedList
		* Loop Invariant : all the elements before "current" is processed and all 
		* the elements after "current" is still be processed
        */
       ...
       /**
        * function to recursively reverse an instance of SinglyLinkedList
		* Loop Invariant : all the elements before "current" is processed and all 
		* the elements after "current" is still be processed
        */
         public void recursiveReverse() {
           recursiveReverse(null, head.next);
         }
           public void recursiveReverse(Entry<T> previous , Entry<T> current ) {
           ...
           if( current == null) {
               ...
               previous = current;
               current = next;
               recursiveReverse(previous, current);
    ...
    //FOR MULTIZIP
    lst.multizip(3);
    System.out.println("Output of Multizip");
    lst.printList();
      
    //FOR REVERSE 
    System.out.println("Output of Iterative Reverse");
    lst.iterativeReverse();
    lst.printList();
    
    System.out.println("Output of Recursive Reverse");
